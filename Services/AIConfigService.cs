    ...
    public class AIConfigService
    {
        ...
        public AIOptions LoadOptions()
        {
            _options = new AIOptions
            {
                ...
                Model = _configService.GetProviderSetting(_alias, "model", string.Empty),
                AsHtml = _configService.GetProviderSetting(_alias, "asHtml", true),
                Prompt = _configService.GetProviderSetting(_alias, "prompt", string.Empty),
                SystemPrompt = _configService.GetProviderSetting(_alias, "systemPrompt", string.Empty),
                Split = _configService.GetProviderSetting(_alias, "split", true),
                Throttle = _configService.GetProviderSetting(_alias, "throttle", 250),
                MaxTokens = _configService.GetProviderSetting(_alias, "maxTokens", 500),
                Temperature = _configService.GetProviderSetting(_alias, "temperature", 0f),
                FrequencyPenalty = _configService.GetProviderSetting(_alias, "frequencyPenalty", 0.0f),
                PresencePenalty = _configService.GetProviderSetting(_alias, "presencePenalty", 0.0f),
                NucleusSamplingFactor = _configService.GetProviderSetting(_alias, "nucleusSamplingFactor", 1),
